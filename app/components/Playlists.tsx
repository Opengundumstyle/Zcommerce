...
const Playlists = ({ playlists,spodify,webPlayer,current_track,context}:any) => {
    ...
    return (
      <div style={{ overflowY: 'scroll', maxHeight: '300px'}}>
        ...
        {playlists.map((playlist: { id: any; images: { url: any; }[]; name: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | ReactFragment | PromiseLikeOfReactNode | null | undefined; }, index: Key | null | undefined) => (
         <div 
          key={index}
           className="hover:bg-slate-500 hover:bg-opacity-60"
           onClick={() => {
           ...
           >
            <div key={index} className="flex flex-row justify-start items-center gap-5 p-2 cursor-pointer">
                ...
                <h3>{playlist.name}</h3>
                {playlist.uri === context.uri && <MusicPlayingAnimation/>}
            </div>
